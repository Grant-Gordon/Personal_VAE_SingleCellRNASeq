...
import yaml
import re
from config.supported_types import SUPPORTED_OBJECTS, SUPPORTED_LAYERS, SUPPORTED_INITIALIZERS
...
def generate_layers_vector(identifier: str, architecture: list) -> list:
    lines = [
        ...
        f"inline std::vector<std::shared_ptr<Layer<Scalar>>> get_{identifier}() {{",
        "    return {"
    ]
    for layer in architecture:
        ...
        if layer_type not in SUPPORTED_LAYERS:
            raise ValueError(f"Layer Type: {layer_type} not found in supported_types.SUPPORTED_LAYERS")
        transformed_args = []
    ...
    for k, v in d.items():
        ...
        if construct_mode == "object":
            ...
            out_lines.extend([
                ...
                f"inline std::unique_ptr<{obj_type}<Scalar>> get_{identifier}() {{",
                f"    return std::make_unique<{obj_type}<Scalar>>({', '.join(map(str, args))});",
                f"}}",
                ""
            ])
            ...
            # Do nothing — these keys are flattened as constexpr scalars
            continue
...
def write_config_header(yaml_path: str, values_out: str = "config_values.h", objects_out: str  = "config_objects.h", external_context: dict = None):
    ...
    with open(objects_out, "w") as f:
        f.write("\n".join(config_object_lines))


if __name__ == "__main__":
    write_config_header("example_config.yaml", "example_config_values.h", "examples_config_objects.h", external_context={"data.num_features": 1000})
