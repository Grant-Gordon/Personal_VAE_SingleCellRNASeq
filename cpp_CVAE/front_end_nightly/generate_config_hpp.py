...
def flatten_config(d: dict, prefix:str="") -> dict:
    flat = {}
    for k, v in d.items():
        if k =="architecture": #Handle seperately
            continue
...
def generate_scalar_lines(flat_config: dict)->list:
    ...
    for cpp_type, cpp_key, cpp_val in entries:
        #padded = f"{cpp_type.ljust(type_width)} {cpp_key.ljust(name_width)} = {cpp_val};" #align values too
        padded = f"{cpp_type.ljust(type_width)} {cpp_key} = {cpp_val};" #align just the names
        lines.append(padded)
    return lines

def generate_layers_vector(architecture: list)-> list:
    lines = [
    "template <typename Scalar>",
    "constexpr std::array<std::shared_ptr<Layer<Scalar>>, {}> layers_vector = {{".format(len(architecture))
    ]
    for layer in architecture:
        if not isinstance(layer, dict) or len(layer) != 1:
            raise ValueError(f"Invalid layer entr: {layer}")
...
def write_config_header(yaml_path:str,  output_path: str= "config.h", external_context:dict = None):
    ...
    architecture_lines = generate_layers_vector(config["model"]["architecture"])
    ...
    full_lines = [
        ...
        "#include <memory>",
        "#include <array>",
        '#include "Layers/LinearLayer.hpp"', #TODO: make Layer/Layer_dir.h that contains all .h files 
        '#include "Layers/ReLULayer.hpp"',  #TODO: make Layer/Layer_dir.h that contains all .h files 
