...
from CVAE.cvae import VaritationalAutoEncoder
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Train Conditional Variational AutoEncoder on single-cell RNA chunks.")
   ...
   #instantiate args
    ...
    model = VaritationalAutoEncoder(input_size).to(device)
    ...
    def preload_worker():
        while True:
            chunk_idx = chunk_queue.get() #Wait up to 3 seconds for work
    ...
    for epoch in range(epochs):
        ...
        epoch_total_kl_loss = 0.0
        epoch_total_recon_loss = 0.0
        ...
        for chunk_idx in range(len(chunks_dataset)):
            ...
            chunk_total_recon_loss = 0.0
            chunk_total_kl_div = 0.0
            ...
            for batch_idx, batch in enumerate(current_loader):
                ...
                batch_loss, reconstructed_loss, kl_div = model.vae_loss(reconstructed, batch, mu, logvar)
                batch_loss.backward()
                optimizer.step()
                ...
                chunk_total_recon_loss +=reconstructed_loss.item()
                chunk_total_kl_div += kl_div.item()
            ...
            chunk_avg_recon_loss = chunk_total_recon_loss / num_batches_in_chunk
            chunk_avg_kl_loss = chunk_total_kl_div / num_batches_in_chunk
            csv_writer.writerow([
                epoch,
                chunk_idx,
                chunk_load_time,
            ...
            tensorboard_writer.add_scalars(f"ChunkStats/Epoch_{epoch}", {
                "Chunk_loss": chunk_avg_loss,
                "Chunk_kl_loss": chunk_avg_kl_loss,
                "Chunl_recon_loss":chunk_avg_recon_loss,
                "Avg_batch_train_time": avg_batch_train_time,
                "Avg_batch_creation_time": avg_batch_creation_time,
        ...
        epoch_avg_recon_loss = epoch_total_recon_loss / num_batches_in_epoch
        epoch_avg_kl_loss = epoch_total_kl_loss / num_batches_in_epoch
        #Tensorboard Epoch Summary
        tensorboard_writer.add_scalars("Epoch_summary", {
            "Epoch_loss": epoch_avg_loss,
            "Epoch_kl_loss":epoch_avg_kl_loss,
            "Epoch_recon_loss":epoch_avg_recon_loss,
            "Epoch_time": epoch_time,
            "Min_chunk_load_time": min(chunk_load_times),
            "Max_chunk_load_time": max(chunk_load_times),
            "Avg_chunk_load_time": sum(chunk_load_times) / len(chunk_load_times),
        }, epoch)
        tensorboard_writer.add_histogram("Batch_train_times", torch.tensor(batch_train_times), epoch)
        tensorboard_writer.add_histogram("Last_chunk/Batch_creation_times", torch.tensor(batch_creation_times), epoch)

        # TensorBoard: text summary (replaces print)
        summary_text = (
            f"Epoch {epoch+1}/{epochs}\n"
            f"Avg Epoch Loss: {epoch_avg_loss:.6f}\n"
            f"Avg Epoch kl Loss: {epoch_avg_kl_loss:.6f}\n"
            f"Avg Epoch Reconstruction Loss: {epoch_avg_recon_loss:.6f}\n"
