#!/bin/bash
#SBATCH --job-name=Train_VAE_1epoch
...
python3 -m CVAE.train_CVAE --data_dir=$DATA_DIR --epochs=$EPOCHS --batch_size=$BATCH_SIZE --output_dir=$OUTPUT_DIR --species=$SPECIES --chunks_preloaded=$CHUNKS_PRELOADED --num_preloader_threads=$NUM_PRELOADER_THREADS
